...
df_src = spark.sql('''
    select distinct Date_ID from parquet.`abfss://silver@saanushridatalake.dfs.core.windows.net/carsales`
    ''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_date'):
    ...
    df_sink = spark.sql('''
        select 1 as dim_date_key, Date_ID from parquet.`abfss://silver@saanushridatalake.dfs.core.windows.net/carsales`
        where 1=0
        ''')
...
# incremental run
if spark.catalog.tableExists('cars_catalog.gold.dim_date'):
    delta_tbl = DeltaTable.forPath(spark,'abfss://gold@saanushridatalake.dfs.core.windows.net/dim_date')
...
# initial run
    ...
    df_final.write.format('delta')\
        .mode("overwrite")\
        .option('path','abfss://gold@saanushridatalake.dfs.core.windows.net/dim_date')\
