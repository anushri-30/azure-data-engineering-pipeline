...
df_src = spark.sql('''
    select distinct Dealer_ID, DealerName from parquet.`abfss://silver@saanushridatalake.dfs.core.windows.net/carsales`
    ''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    ...
    df_sink = spark.sql('''
        select 1 as dim_dealer_key, Dealer_ID, DealerName from parquet.`abfss://silver@saanushridatalake.dfs.core.windows.net/carsales`
        where 1=0
        ''')
...
# incremental run
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    delta_tbl = DeltaTable.forPath(spark,'abfss://gold@saanushridatalake.dfs.core.windows.net/dim_dealer')
...
# initial run
    ...
    df_final.write.format('delta')\
        .mode("overwrite")\
        .option('path','abfss://gold@saanushridatalake.dfs.core.windows.net/dim_dealer')\
